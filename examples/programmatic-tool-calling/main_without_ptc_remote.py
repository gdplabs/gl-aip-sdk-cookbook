"""Scenario demo without Programmatic Tool Calling (PTC) - Remote run."""
...
agent = Agent(
    name=f"budget_audit_without_ptc_{uuid.uuid4().hex[:6]}",
...
agent.deploy()
try:
    agent.run(query, local=False, verbose=True)
finally:
    agent.delete()
